...
This repo makes two changes to the original repo from moodlehq.
Firstly, it adds the maxima server, which should have no impact
on basic moodle functionality.  
Secondly, it modifies the main webserver to make YAML available.
This could conceivably negatively impact the basic moodle service.
If it is not required, the following lines in `base.yml` may be changed:

```bash
services:
  webserver:
    image: "stack-php-apache:${MOODLE_DOCKER_PHP_VERSION}"
```

The `image:` line chooses the image for the web server.
This image, `stack-php-apache` is the one generated by the `build.sh` script.
To use the official moodlehq image, change it to this:
...
### Upgrades of the underlying docker images

The current `build.sh` makes a docker image using PHP 7.3.
It is possible that upgrades of the moodlehq scripts will require different
versions of the apache/php image.  The version can be controlled with the
`$MOODLE_DOCKER_PHP_VERSION` environment variable.  Read the script for
further information.
